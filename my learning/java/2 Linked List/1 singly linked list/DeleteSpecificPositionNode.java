public class DeleteSpecificPositionNode {
  ...
  public ListNode deleteSpecificPositionNode(int position) {

    ListNode previous = head;
    ListNode nodeAtPosition = null;

    if (position > length()) {
      return nodeAtPosition; // will throw an error as there will be no data property so that should be a
                             // minor edge case
    }

    if (position == 1) {
      head = previous.next;
      nodeAtPosition = previous;
      ...
      nodeAtPosition = previous.next;
      ListNode nextNode = nodeAtPosition.next;
      nodeAtPosition.next = null;
      previous.next = nextNode;

    }
    // deleted node will be garbage collected
    return nodeAtPosition;
  }

  public static void main(String[] args) {
    DeleteSpecificPositionNode sll = new DeleteSpecificPositionNode();
    sll.insertEnd(6);
    sll.insertEnd(7);
    sll.insertEnd(8);
    sll.insertEnd(9);
    ...
    System.out.println("Deleted node is: " + sll.deleteSpecificPositionNode(3).data);
    System.out.println("Deleted node is: " + sll.deleteSpecificPositionNode(1).data);
    System.out.println("Deleted node is: " + sll.deleteSpecificPositionNode(1).data);
    // System.out.println("Deleted node is: " +
    // sll.deleteSpecificPositionNode(7).data);
