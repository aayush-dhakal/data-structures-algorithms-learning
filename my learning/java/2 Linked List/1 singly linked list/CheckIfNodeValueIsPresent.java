public class CheckIfNodeValueIsPresent {
  ...
  public boolean checkIfNodeIsPresent(int value) {
    ListNode current = head; // temp variable to traverse to last node
    while (current.next != null) { // current.next will be null for last node
      ...
      current = current.next; // move the current pointer to next node
    }
  ...
  public static void main(String[] args) {
    CheckIfNodeValueIsPresent sll = new CheckIfNodeValueIsPresent();
    sll.insertEnd(6);
    sll.insertEnd(7);
    sll.insertEnd(8);
    sll.insertEnd(9);
    ...
    System.out.println("is value present: " + sll.checkIfNodeIsPresent(74));
